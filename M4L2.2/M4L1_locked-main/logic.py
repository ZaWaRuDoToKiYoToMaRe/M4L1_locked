...
class DatabaseManager:
    ...
    def get_winners_count(self, prize_id):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT * FROM users WHERE date_id = ?', (prize_id, ))
            return cur.fetchall()[0][0]
    def get_winners_count(self, prize_id):
        conn = sqlite3.connect(self.database)
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT * FROM users WHERE prize_id = ?', (prize_id, ))
            return cur.fetchall()[0][0]
    def get_winners_count(self, prize_id):
        conn = sqlite3.connect(self.database)
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT * FROM users WHERE users_id = ?', (prize_id, ))
            return cur.fetchall()[0][0]     
    ...
    def get_winners_count(self, prize_id):
        ...
        with conn:
            cur = conn.cursor()
            cur.execute('SELECT * user_name WHERE prize_id = ?', (prize_id, ))
            return cur.fetchall()[0][0]
